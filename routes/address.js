...
router.get('/get/ward', (request,response, next) =>{
  pool.query("Select wards.ward_id, wards.ward_no,wards.nagarsevak ,city.city_id, city.city, mc_list.mc_id, mc_list.mc ,district.district_id,district.district, state.state_id, state.state, state.country_id, country.country from state INNER JOIN country on state.country_id = country.country_id as state inner join district on district.state_id = state.state_id as district as district INNER JOIN mc_list on mc_list.state_id = state.state_id as mc inner join city on mc_list.mc_id = city.city_id as city inner join wards on wards.ward_id = mc_list.mc_id as wards", (err, res) =>{
      if(err) return next(err);
      response.json({
           "message":"success",
            "data":res.rows});
